...
MODEL_NAME = "openai/gpt-oss-120b:free"  # Free version
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if attempt == max_retries - 1:
                    st.error(f"OpenRouter API error (status {response.status_code}): {error_detail}")
                    return None
                time.sleep(2 ** attempt)  # exponential backoff
        except requests.exceptions.Timeout:
            if attempt == max_retries - 1:
                st.error("OpenRouter API timeout after retries.")
                return None
            time.sleep(2 ** attempt)
        except Exception as e:
            if attempt == max_retries - 1:
                st.error(f"OpenRouter API exception: {str(e)}")
                return None
            time.sleep(2 ** attempt)
    return None
...
def extract_claims(pdf_text, max_claims=15):
    """Use OpenRouter to extract verifiable claims. Robust JSON parsing."""
    prompt = f"""
...
"""
    ...
    try:
        ...
        st.warning(f"OpenRouter returned malformed JSON. Attempting repair... {str(e)}")
        import re
...
def verify_claim(claim_text):
    """
    Search web via Tavily, then use OpenRouter to evaluate claim.
    Web search results are the primary source of truth.
...
"""
    ...
    if not response:
        return {
            "verdict": "False",
            "correct_fact": "Verification failed due to API error.",
            "explanation": "Could not get response from OpenRouter.",
            "confidence": "Low",
            "sources": sources[:2]
        }
    ...
    try:
        ...
        return {
            "verdict": "False",
            "correct_fact": "Verification failed due to parsing error.",
